---
layout: post
title: "[Python]Datapane Report to Pdf"
excerpt: Python Datapane로 작성한 Report를 pdf로 변환해보자.
categories:
    - Python
...
&nbsp; [지난 포스트](https://sparkafka.github.io/7-python-datapane/)에서 Python Datapane 라이브러리를 사용해 HTML 형식의 Report를 작성하는 법을 알아보았다. 나는 Datapane으로 작성한 Report를 pdf로 변환하기 위해 파이썬에서 HTML을 pdf로 바꿔주는 라이브러리들을 찾아보았는데, 하나같이 변환이 안 되는 것이었다! 그래서 나는 다른 방법을 찾아보았다...

<br/><br/>

## 1. 기존의 HTML to pdf 라이브러리

&nbsp; 처음에 나는 HTML 파일을 pdf로 변환하기 위해 많이 사용되는 [wkhtmltopdf](https://wkhtmltopdf.org/)를 이용하는 python 라이브러리인 [pdfkit](https://github.com/JazzCore/python-pdfkit) 라이브러리를 사용하였다. 지난 포스트에서 생성한 table-test.html 파일을 변환해보자.
...
![pdfkit example](/images/8th/pdfkit-example.png)

&nbsp; 아무 내용도 없이 빈 화면만 보인다. 다른 [weasyprint](https://weasyprint.org/) 같은 라이브러리도 똑같이 변환이 안 되었다. 원인은 무엇일까? 다음은 table-test.html 파일의 일부분이다.<br/>

![html insde](/images/8th/html-inside.png)

&nbsp; 그림과 같이 간단한 표 하나만 있는 Report 이지만 상당히 많은 내용이 있는 것을 알 수 있다. 그리고 그 내용의 대부분은 script 이다. 내가 프론트쪽은 지식이 없어서 잘은 모르지만, 이것이 기존의 변환 라이브러리가 작동하지 않는 원인이지 않을까? <br/>

&nbsp; 그래서 나는 다른 방법을 찾아야 했다. 어떻게 해야되나 고민하다가, 'pdf로 바로 변환이 안되는 거면, html을 이미지 파일로 만들고 이미지 파일을 pdf로 바꾸면 되지 않을까?' 라는 생각을 하였다. 그래서 나는 그 생각을 실행해 보았다.
